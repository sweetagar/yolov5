...
def train(hyp,
    ...
    # Hyperparameters超参
    if isinstance(hyp, str): #用于检查变量hyp是否是字符串
        with open(hyp) as f:
            hyp = yaml.safe_load(f) # load hyps dict  加载超参信息 data/hyps/hyp.scratch.yaml
    ...
    init_seeds(2 + RANK)
    # data_dict: 加载data/coco128.yaml中的数据配置信息  dict
    with open(data) as f: 
    ...
    if pretrained:
        ...
        model.load_state_dict(state_dict, strict=True)  # load #strict：是否严格要求state_dict中的键与该模块的键返回的键匹配
    ...
    if pretrained:
        #这里因为低bit模型相对于32bit模型不是接着训练，而是类似于重训，所以建议去掉加载optimizer，ema部分，以免影响低bit的训练效果
        # Optimizer
    ...
    t0 = time.time()
    # 获取热身迭代的次数iterations  
    # 学习率预热：在训练开始时，将学习率设置为一个较小的初始值，然后逐渐增加学习率的大小。这样做的目的是使模型在初始阶段更稳定地收敛，避免训练开始时权重变化过大导致模型无法收敛或训练不稳定的情况
    ...
    for epoch in range(start_epoch, epochs):  # epoch
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch
            ...
            # Multi-scale:false 多尺度训练   从[imgsz*0.5, imgsz*1.5+gs]间随机选取一个尺寸(32的倍数)作为当前batch的尺寸送入模型开始训练
            ...
            if ni % accumulate == 0:
                ...
                optimizer.step()    # optimizer.step   参数更新
            ...
            if RANK in [-1, 0]:
                ...
                if plots and ni < 3:
                    f = save_dir / f'train_batch{ni}.jpg'  # filename 保存数据集输入train_batch图片
                ...
                # wandb 显示信息            
                    ...
                    wandb_logger.log({'Mosaics': [loggers['wandb'].Image(str(x), caption=x.name) for x in
                                                  save_dir.glob('train*.jpg') if x.exists()]}) # glob 文件通配符,遍历目录下的train*.jpg
        ...
        if RANK in [-1, 0]:
            ...
            if not notest or final_epoch:  # Calculate mAP
                ...
                results, maps, _ = test.run(data_dict,# 数据集配置文件地址 包含数据集的路径、类别个数、类名、下载地址等信息
                                            batch_size=batch_size // WORLD_SIZE * 2, # bs
                                            imgsz=imgsz_test,   # test img size
                                            model=ema.ema if use_ema else model, # ema model
...
def parse_opt(known=False):
    parser = argparse.ArgumentParser() #返回的就是self
    # --------------------------------------------------- 常用参数 ---------------------------------------------
    parser.add_argument('--weights', type=str, default='yolov5s.pt', help='initial weights path')               #weights: 权重文件
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')                                  #cfg: 模型配置文件 包括nc、depth_multiple、width_multiple、anchors、backbone、head等
    parser.add_argument('--data', type=str, default='data/coco128.yaml', help='dataset.yaml path')              #data: 数据集配置文件 包括path、train、val、test、nc、names、download等
    parser.add_argument('--hyp', type=str, default='data/hyp.scratch.yaml', help='hyperparameters path')        #hyp: 初始超参文件
    parser.add_argument('--epochs', type=int, default=300)                                                      #epochs: 训练轮次
    parser.add_argument('--batch-size', type=int, default=8, help='total batch size for all GPUs')              #batch-size: 训练批次大小
    ...
    parser.add_argument('--nosave', action='store_true', help='only save final checkpoint')                     #nosave: 不保存模型  默认False(保存)      True: only test final epoch
    parser.add_argument('--notest', action='store_true', help='only test final epoch')                          #notest: 是否只测试最后一轮 默认False  True: 只测试最后一轮   False: 每轮训练完都测试mAP
    ...
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters')                         #evolve: 是否进行超参进化 默认False
    # --------------------------------------------------- 其他参数 ---------------------------------------------
    ...
    parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training')#image-weights: 是否使用图片采用策略(selection img to training by class weights) 默认False 不使用
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')                   #device: 训练的设备
    parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%')                    #multi-scale: 是否使用多尺度训练 默认False
    ...
    parser.add_argument('--workers', type=int, default=8, help='maximum number of dataloader workers')          #workers: dataloader中的最大work数（线程个数）
    ...
    parser.add_argument('--linear-lr', action='store_true', help='linear LR')                                   #linear-lr: 是否使用linear lr  线性学习率  默认False 使用cosine lr
...
def main(opt):
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        # 不使用断点续训 就从文件中读取相关参数 cfg=models/yolov5s.yaml, data=data/coco128.yaml, hyp=data/hyp.scratch.yaml
        # opt.hyp = opt.hyp or ('hyp.finetune.yaml' if opt.weights else 'hyp.scratch.yaml')
        opt.data, opt.cfg, opt.hyp = check_file(opt.data), check_file(opt.cfg), check_file(opt.hyp)  # check files
        assert len(opt.cfg) or len(opt.weights), 'either --cfg or --weights must be specified'
        # 将opt.img_size扩展为[train_img_size, test_img_size] = [640, test_img_size]
