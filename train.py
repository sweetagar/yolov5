...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Hyperparameters
    if isinstance(hyp, str): #isinstance函数来判断一个对象是否是一个已知的类型，类似 type()
        with open(hyp) as f:
    ...
    if RANK in [-1, 0]:
        ...
        # W&B wandb是Weights & Biases的缩写，是类似TensorBoard， visdom的一款可视化工具
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni % accumulate == 0:
                ...
                # scaler.update()
                # 等于手动更新parameters().data.add_(-learning_rate, p.grad.data)   【p=p-lr*grad】
                optimizer.step()
                optimizer.zero_grad()
                if use_ema and ema:
                    ema.update(model)
...
def main(opt):
    #logging.basicConfig 设置日志显示级别 level=logging.INFO
    set_logging(RANK) 
    if RANK in [-1, 0]:
