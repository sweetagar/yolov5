...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.hyp = hyp #data/hyp.scratch.yaml
        ...
        # Read cache 定义训练用到的参数
        [cache.pop(k) for k in ('hash', 'version', 'msgs')]  # remove items
        labels, shapes, self.segments = zip(*cache.values())
        self.labels = list(labels)
        self.shapes = np.array(shapes, dtype=np.float64)
        self.img_files = list(cache.keys())  # update 图片路径
        self.label_files = img2label_paths(cache.keys())  # update label路径
        if single_cls:
            for x in self.labels:
                x[:, 0] = 0

        n = len(shapes)  # number of images 没涨图片shape
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)  # batch index，第几个batch
        nb = bi[-1] + 1  # number of batches 每一个batch多少
        self.batch = bi  # batch index of image
        self.n = n
        self.indices = range(n)

        # Rectangular Training 矩形训练/推理方式（就是变成640x640，再去除黑边，相当于padding）（优点：训练过程计算量）
        if self.rect:
            # Sort by aspect ratio 重新计算图片宽高比
            s = self.shapes  # wh
            ...
            # Set training image shapes 重新计算shape
            shapes = [[1, 1]] * nb
            for i in range(nb):
                ari = ar[bi == i]
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        x['results'] = nf, nm, ne, nc, len(self.img_files) #解析结果
        x['msgs'] = msgs  # warnings
        x['version'] = 0.3  # cache version
        try:
            torch.save(x, path)  # save cache for next time 保存在本地
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    # Combined rotation matrix 越重要的放到越后面
    M = T @ S @ R @ P @ C  # order of operations (right to left) is IMPORTANT
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            img = cv2.warpPerspective(img, M, dsize=(width, height), borderValue=(114, 114, 114))
        else:  # affine 仿射变换（能够保证原本平行的边，转换完后平行）
