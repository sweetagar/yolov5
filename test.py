...
def run(data,                                                                 #data数据集配置文件地址 coco128.yaml
        weights=None,  # model.pt path(s)                                     #weights模型的权重文件地址 运行train.py=None 运行test.py=默认weights/yolov5s.pt
        batch_size=32,  # batch size                                          #batch_size: 前向传播的批次大小 运行test.py传入默认32 运行train.py则传入batch_size // WORLD_SIZE * 2
        imgsz=640,  # inference size (pixels)                                 #imgsz: 输入网络的图片分辨率 运行test.py传入默认640 运行train.py则传入imgsz_test
        conf_thres=0.001,  # confidence threshold                               #conf_thres: object置信度阈值 默认0.25
        iou_thres=0.6,  # NMS IoU threshold                                     #iou_thres: 进行NMS时IOU的阈值 默认0.6
        task='val',  # train, val, test, speed or study                         #task: 设置测试的类型 有train, val, test, speed or study几种 默认val
        device='',  # cuda device, i.e. 0 or 0,1,2,3 or cpu                     #device: 测试的设备
        single_cls=False,  # treat as single-class dataset                      #single_cls: 数据集是否只用一个类别 运行test.py传入默认False 运行train.py则传入single_cls
        augment=False,  # augmented inference                                   #augment: 测试是否使用TTA Test Time Augment 默认False
        verbose=False,  # verbose output                                        #verbose: 是否打印出每个类别的mAP 运行test.py传入默认Fasle 运行train.py则传入nc < 50 and final_epoch
        save_txt=False,  # save results to *.txt                                #save_txt: 是否以txt文件的形式保存模型预测框的坐标 默认False
        save_hybrid=False,  # save label+prediction hybrid results to *.txt     #save_hybrid:是否save label+prediction hybrid results to *.txt  默认False 是否将gt_label+pre_label一起输入nms
        save_conf=False,  # save confidences in --save-txt labels               #save_conf: 是否保存预测每个目标的置信度到预测tx文件中 默认True
        save_json=False,  # save a cocoapi-compatible JSON results file         #save_json: 是否按照coco的json格式保存预测框，并且使用cocoapi做评估（需要同样coco的json格式的标签）,运行test.py传入默认Fasle 运行train.py则传入is_coco and final_epoch(一般也是False)
        project='runs/test',  # save to project/name                            #project: 测试保存的源文件 默认runs/test
        name='exp',  # save to project/name                                     #name: 测试保存的文件地址 默认exp  保存在runs/test/exp下
        exist_ok=False,  # existing project/name ok, do not increment           #exist_ok: 是否存在当前文件 默认False 一般是 no exist-ok 连用  所以一般都要重新创建文件夹
        half=False,  # use FP16 half-precision inference                        #half: 是否使用半精度推理 FP16 half-precision inference 默认False
        model=None,                                                             #model: 模型 如果执行test.py就为None 如果执行train.py就会传入ema.ema(ema模型)
        dataloader=None,                                                        #dataloader: 数据加载器 如果执行test.py就为None 如果执行train.py就会传入testloader
        save_dir=Path(''),                                                      #save_dir: 文件保存路径 如果执行test.py就为‘’ 如果执行train.py就会传入save_dir(runs/train/expn)
        plots=True,                                                             # plots: 是否可视化 运行test.py传入默认True 运行train.py则传入plots and final_epoch
        wandb_logger=None,                                                      #wandb_logger: 网页可视化 类似于tensorboard 运行test.py传入默认None 运行train.py则传入wandb_logger(train)
        compute_loss=None,                                                      #compute_loss: 损失函数 运行test.py传入默认None 运行train.py则传入compute_loss(train)
        ):
        ...
        for si, pred in enumerate(out):        
            ...
            if save_txt:
                ...
                for *xyxy, conf, cls in predn.tolist():
                    ...
                    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
                    with open(save_dir / 'labels' / (path.stem + '.txt'), 'a') as f: #获取文件路径中的文件名部分
